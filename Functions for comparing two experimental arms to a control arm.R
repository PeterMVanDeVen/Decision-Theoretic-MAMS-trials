...
# This script contains all functions required for the setting where
# two experimental arms are compared to a control arm
#
...
#             observed data (specific for setting with control arm and two
#             experimental arms)
# Ttheta    : Determines the correct decision given a true parameter vector theta
#             (specific for setting with control arm and two experimental arms)
...
# gamma   : Minimum increase in probability of a correct decision  
#           required for continuation of the trial (=C/Q in paper)
# burn    : Number of subjects per arm for stage 1 
...
prnodrop <- function(Y, delta = 0, gamma, burn = 0, Ntrials, batch, prior, maxpt){
  ...
  LL <- foreach(i=1:Ntrials, .combine = rbind, .export=c('evalnodrop', 'Ttheta', 'Tdata')) %dopar% {
    ...
    c(len, dat, list(benefit = ben, drop=isdr, nstage = ns))
  }
  print(Sys.time())
  stopCluster(cl)
  rm(cl)
...
# prdrop or prnodrop function using a higher threshold gamma (=C/Q in paper) for continuation
...
# newgamma: New minimally required increase in probability  
#           of correct decision required for continuation (C/Q in paper) 
#
...
#           superior to the control arm (arm 1). Should be the same value used as input for
#           prdrop and prnodrop functions
...
# The function included here is for the loss function that appears as equation (3)
# in the manuscript and can be used when comparing two experimental arms to a control arm.
#
# Required input:
#
# y      : Vector with number of successes observed so far in the trial
...
Tdata <- function(y, n, prior, delta = 0){
...
#
# Required input:
# theta  : Vector containing response/success probabilities for the three arms 
# delta  : Margin (delta) above which experimental arms (arms 2,3) are considered   
#           superior to the control arm (arm 1).   
#

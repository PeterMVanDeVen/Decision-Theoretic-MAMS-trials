...
Tdata <- function(y, n, prior, delta = 0){
  ...
  if(K==3) {prob <- c(mean(theta[,1]-theta[,2] >= 0 & theta[,1]-theta[,3] >  0), 
            mean(theta[,2]-theta[,1] >  0 & theta[,2]-theta[,3] >= 0),
            mean(theta[,3]-theta[,1] >= 0 & theta[,3]-theta[,2] >  0))  } 

  if(K==4) {prob <- c(mean(theta[,1]-theta[,2] >= 0 & theta[,1]-theta[,3] >  0  & theta[,1]-theta[,4] >= 0), 
            mean(theta[,2]-theta[,1] >  0 & theta[,2]-theta[,3] >= 0  & theta[,2]-theta[,4] >  0),
            mean(theta[,3]-theta[,1] >= 0 & theta[,3]-theta[,2] >  0  & theta[,3]-theta[,4] >= 0),
            mean(theta[,4]-theta[,1] >  0 & theta[,4]-theta[,2] >= 0  & theta[,4]-theta[,3] >  0)) }

  if(K==5) {prob <- c(mean(theta[,1]-theta[,2] >= 0 & theta[,1]-theta[,3] >  0  & theta[,1]-theta[,4] >= 0 & theta[,1]-theta[,5] >  0), 
               mean(theta[,2]-theta[,1] >  0 & theta[,2]-theta[,3] >= 0  & theta[,2]-theta[,4] >  0 & theta[,2]-theta[,5] >= 0),
               mean(theta[,3]-theta[,1] >= 0 & theta[,3]-theta[,2] >  0  & theta[,3]-theta[,4] >= 0 & theta[,3]-theta[,5] >  0),
               mean(theta[,4]-theta[,1] >  0 & theta[,4]-theta[,2] >= 0  & theta[,4]-theta[,3] >  0 & theta[,4]-theta[,5] >= 0),
               mean(theta[,5]-theta[,1] >= 0 & theta[,5]-theta[,2] >  0  & theta[,5]-theta[,3] >= 0 & theta[,5]-theta[,4] >  0)) }
